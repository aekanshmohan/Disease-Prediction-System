#!/usr/bin/env python3
"""
Disease Symptom Checker — Single-role (Users only)

This simplified script keeps everything you asked for but removes all
medical-officer related code and tables. It's email-only login for users,
provides symptom checking, history, and a user-driven support request system.

Notes:
- No medical officer, so support requests are stored but not resolved by an officer.
- DB migration still removes legacy password columns on users if present.
- Keep in mind: email-only login is for demo/learning purposes only.
"""
...
def placeholders(n: int) -> str:
    if n <= 0:
        raise ValueError("placeholders requires n >= 1")
    return ','.join(['%s'] * n)
...
def setup_database(conn):
    ...
    try:
        ...
        ds_map = {
            ...
            "Ear Infection": ["Ear pain","Fever","Hearing reduction","Dizziness"],
        ...
        for disease_name, meds in meds_map.items():
            ...
            did = r[0]
            for m in meds:
                cur.execute("INSERT IGNORE INTO disease_medications (disease_id, medication_text) VALUES (%s, %s)", (did, m))
        conn.commit()
...
def list_symptoms(conn) -> List[Tuple[int, str]]:
    cur = conn.cursor()
    try:
...
def get_medications_for_disease(conn, disease_id: int) -> List[str]:
    cur = conn.cursor()
    try:
...
def diagnose(conn, user_id: int, selected_symptom_ids: List[int]) -> List[dict]:
    cur = conn.cursor()
    try:
        ...
        cur.execute("INSERT INTO diagnosis_history (user_id, selected_symptoms, possible_diseases, match_percentages) VALUES (%s,%s,%s,%s)", (user_id, ', '.join(symptom_names), possible_diseases_str, match_percentages_str))
        conn.commit()
        return final
    finally:
        cur.close()
...
def submit_support(conn, user_id: int, symptoms_text: str, system_suggestions: str):
    ...
    try:
        ...
        note = input().strip()
        cur.execute("INSERT INTO support_requests (user_id, symptoms_text, system_suggestions, user_message) VALUES (%s,%s,%s,%s)", (user_id, symptoms_text, system_suggestions, note))
        conn.commit()
...
def view_user_history(conn, user_id: int):
    cur = conn.cursor()
    try:
        cur.execute("SELECT history_id, selected_symptoms, possible_diseases, match_percentages, date_time FROM diagnosis_history WHERE user_id=%s ORDER BY date_time DESC", (user_id,))
        rows = cur.fetchall()
        if not rows:
    ...
    while True:
        ...
        if choice == '1':
            ...
            for sid, sname in symptoms:
                print(f" {sid}. {sname}")
            raw = input("\nEnter symptom numbers separated by commas (e.g. 1,4,12): ").strip()
            try:
...
def main():
    conn = connect_db()
    setup_database(conn)
    print("\nWelcome to the Disease Symptom Checker (Users only)")
    while True:
        print("\nMain Menu:\n1) Register\n2) Login\n3) Exit")
        choice = input("Choice: ").strip()
        if choice == '1':
